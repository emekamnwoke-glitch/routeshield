...
**A route status card added to v1.2.0** (recorded 2026-09-18, at the author's request). A controller needs to see at a glance which bus numbers a closure touches and which are already diverted. The card lists each affected route once: yellow while its bypass awaits approval, red once approved and in effect. It reads data v1.2.0 already produces, so it adds no new component work.

**The first bypass runs in the core worker.** ADR-0006 puts the optimiser in its own time-boxed worker. v1.2.0 runs the single-pair search in the core worker, bounded by a length cap per search, and v1.3.0 moves it out when the search grows to several ranked pairs.
