...
#            4               way 2 (2-4)
STREET = (
    node(1, 53.35, -6.30), node(2, 53.35, -6.295), node(3, 53.35, -6.29), node(4, 53.345, -6.295),
...
def test_path_turns_at_the_junction() -> None:
    ...
    assert all(g.head(a) == g.tail(b) for a, b in zip(path, path[1:]))
...
def test_short_excursions_off_the_graph_are_skipped_not_broken() -> None:
    ...
    path, where, bridged = network.match(
        g,
        shape((53.35003, -6.2995), (53.35003, -6.2972), (53.3518, -6.2964),
              (53.3518, -6.2944), (53.35003, -6.2936), (53.35003, -6.2905)),
    )
    assert edge_ways(g, path) == [(1, True), (1, True)]
    assert bridged == 0
...
def test_paths_are_continuous_and_stops_are_in_order(committed) -> None:
    ...
    for p in net["patterns"]:
        assert all(g.head(a) == g.tail(b) for a, b in zip(p["path"], p["path"][1:])), p["id"]
...
def test_edge_index_finds_the_edges_under_a_stop(committed) -> None:
    ...
    for p in net["patterns"][:5]:
        for s, (k, _, d) in zip(p["stops"], p["stop_positions"]):
